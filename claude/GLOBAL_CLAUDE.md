...
- The shared profile's roles exist as subagents of the same names in `~/.claude/agents/`: `luna_worker` (Sonnet, medium effort; pass `model: haiku` for a purely mechanical chore or `opus` for harder bounded analysis), `critical_reviewer` (Fable, high) and `professional_reviewer` (Fable, max).
